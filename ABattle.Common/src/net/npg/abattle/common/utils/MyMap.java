 ...
 * 
 * <p>
 * This interface takes the place of the <tt>Dictionary</tt> class, which was a totally abstract class rather than an
 * interface.
 * 
 * <p>
 * The <tt>Map</tt> interface provides three <i>collection views</i>, which allow a map's contents to be viewed as a set
 * of keys, collection of values, or set of key-value mappings. The <i>order</i> of a map is defined as the order in
 * which the iterators on the map's collection views return their elements. Some map implementations, like the
 * <tt>TreeMap</tt> class, make specific guarantees as to their order; others, like the <tt>HashMap</tt> class, do not.
 * 
 * <p>
 ...
 * constructor which creates an empty map, and a constructor with a single argument of type <tt>Map</tt>, which creates
 * a new map with the same key-value mappings as its argument. In effect, the latter constructor allows the user to copy
 * any map, producing an equivalent map of the desired class. There is no way to enforce this recommendation (as
 * interfaces cannot contain constructors) but all of the general-purpose map implementations in the JDK comply.
 * 
 * <p>
 ...
 * invocation would have no effect on the map. For example, invoking the {@link #putAll(Map)} method on an unmodifiable
 * map may, but is not required to, throw the exception if the map whose mappings are to be "superimposed" is empty.
 * 
 * <p>
 ...
 * <tt>(key==null ? k==null : key.equals(k))</tt>." This specification should <i>not</i> be construed to imply that
 * invoking <tt>Map.containsKey</tt> with a non-null argument <tt>key</tt> will cause <tt>key.equals(k)</tt> to be
 * invoked for any key <tt>k</tt>. Implementations are free to implement optimizations whereby the <tt>equals</tt>
 * invocation is avoided, for example, by first comparing the hash codes of the two keys. (The {@link Object#hashCode()}
 * specification guarantees that two objects with unequal hash codes cannot be equal.) More generally, implementations
 * of the various Collections Framework interfaces are free to take advantage of the specified behavior of underlying
...
public interface MyMap<K, V> {
	 ...
	 * returned by the iterator, except through the <tt>setValue</tt> operation on the map entry.
	 * 
	 * @see Map#entrySet()
	 * @since 1.2
	 */
	interface Entry<K, V> {
		 ...
		 * @throws ClassCastException if the class of the specified value prevents it from being stored in the backing
		 *             map
		 * @throws NullPointerException if the backing map does not permit null values, and the specified value is null
		 * @throws IllegalArgumentException if some property of this value prevents it from being stored in the backing
		 *             map
		 * @throws IllegalStateException implementations may, but are not required to, throw this exception if the entry
		 *             has been removed from the backing map.
		 */
